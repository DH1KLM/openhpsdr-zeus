; Zeus Installer Script for Windows ARM64
; Requires Inno Setup 6.3 or later (IsARM64 check): https://jrsoftware.org/isinfo.php
...
[Setup]
AppId={{8F2E3B1C-9A4D-4E6F-B7C3-1D5A9E8F2B4D}
AppName={#MyAppName}
...
LicenseFile=..\LICENSE
OutputDir=.\output
OutputBaseFilename=Zeus-{#MyAppVersion}-win-arm64-setup
Compression=lzma2
...
[Files]
Source: "..\Zeus.Server\bin\Release\net10.0\win-arm64\publish\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
begin
  ...
  if not IsARM64 then
  begin
    MsgBox('This installer is for Windows on ARM64 only.', mbError, MB_OK);
    Result := False;
  end;
end;
